<?php
/**
 * SCRIPT PARA IMPORTAR DATOS DEL SISTEMA VIEJO AL CATÁLOGO
 * Ejecutar desde: https://aramedylaboratorio.com/NUEVO/aramed/public_html/importar_datos_catalogo.php
 */
...
// Función para importar datos de productos
function importProductsData($pdo) {
    ...
    try {
        ...
        $pdo->exec("TRUNCATE TABLE productos_temp");
        
        // Extraer solo los INSERT statements
        preg_match_all('/INSERT INTO `productos`[^;]+;/', $sql, $matches);
        ...
        foreach ($matches[0] as $insert) {
            try {
                ...
                echo "⚠️ Error importando producto: " . substr($insert, 0, 100) . "...<br>";
            }
        }
...
// Función para importar datos de imágenes
function importImagesData($pdo) {
    ...
    try {
        // Limpiar tabla temporal
        $pdo->exec("TRUNCATE TABLE imagenes_x_producto_temp");
        
        // Extraer solo los INSERT statements
        preg_match_all('/INSERT INTO `imagenes_x_producto`[^;]+;/', $sql, $matches);
        ...
        foreach ($matches[0] as $insert) {
            try {
                ...
                echo "⚠️ Error importando imagen: " . substr($insert, 0, 100) . "...<br>";
            }
        }
...
// Función para migrar datos a las nuevas tablas
function migrateToNewTables($pdo) {
    echo "<br>🔄 MIGRANDO A NUEVAS TABLAS DEL CATÁLOGO...<br>";
    echo str_repeat("-", 50) . "<br>";
    
    try {
        // Limpiar tablas del catálogo
        $pdo->exec("TRUNCATE TABLE catalogo_productos");
        $pdo->exec("TRUNCATE TABLE catalogo_producto_imagenes");
        $pdo->exec("TRUNCATE TABLE catalogo_producto_documentos");
        ...
        SELECT 
            p.sku as codigo,
            p.titulo as nombre,
            LOWER(REPLACE(REPLACE(REPLACE(p.titulo, ' ', '-'), 'á', 'a'), 'é', 'e')) as slug,
            LEFT(p.descripcion, 500) as descripcion_corta,
        ...
        WHERE p.titulo IS NOT NULL AND p.titulo != ''
        ";
...
echo "<h1>🚀 IMPORTANDO DATOS DEL SISTEMA VIEJO AL CATÁLOGO</h1>";
echo "<hr>";
