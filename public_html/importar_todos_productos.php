<?php
/**
 * SCRIPT PARA IMPORTAR TODOS LOS PRODUCTOS DEL SISTEMA VIEJO AL CATÁLOGO
 * Migra todos los productos del archivo productos.sql
 */
...
function importAllProducts($pdo) {
    ...
    try {
        ...
        // Extraer todas las líneas que contienen INSERT
        $lines = explode("\n", $sql);
        ...
        foreach ($lines as $line_num => $line) {
            ...
            // Buscar líneas que contengan VALUES con datos de productos
            if (strpos($line, 'VALUES') !== false && strpos($line, 'INSERT INTO') === false) {
                // Extraer valores usando regex más específico
...
function importImages($pdo) {
    ...
    try {
        ...
        foreach ($lines as $line_num => $line) {
            ...
            // Buscar líneas que contengan VALUES con datos de imágenes
            if (strpos($line, 'VALUES') !== false && strpos($line, 'INSERT INTO') === false) {
...
echo "<h1>🚀 IMPORTANDO TODOS LOS PRODUCTOS DEL SISTEMA VIEJO AL CATÁLOGO</h1>";
echo "<hr>";
