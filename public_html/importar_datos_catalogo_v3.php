<?php
/**
 * SCRIPT V3 PARA IMPORTAR DATOS DEL SISTEMA VIEJO AL CATÁLOGO
 * Maneja caracteres especiales y HTML en las descripciones
 */
...
// Función para importar datos de productos (versión mejorada)
    ...
    try {
        ...
        foreach ($matches[0] as $insert) {
            try {
                ...
                // Decodificar entidades HTML problemáticas
                $insert = str_replace('&Iacute;', 'Í', $insert);
                $insert = str_replace('&Ntilde;', 'Ñ', $insert);
                $insert = str_replace('&Aacute;', 'Á', $insert);
                $insert = str_replace('&Eacute;', 'É', $insert);
                $insert = str_replace('&Oacute;', 'Ó', $insert);
                $insert = str_replace('&Uacute;', 'Ú', $insert);
                $insert = str_replace('&ntilde;', 'ñ', $insert);
                $insert = str_replace('&aacute;', 'á', $insert);
                $insert = str_replace('&eacute;', 'é', $insert);
                $insert = str_replace('&iacute;', 'í', $insert);
...
function migrateToNewTables($pdo) {
    ...
    try {
        // Migrar productos usando prepared statements para evitar problemas de caracteres
        $stmt = $pdo->prepare("
        ...
        $stmt->execute();
        $productos_migrados = $stmt->rowCount();
        echo "✅ Productos migrados: $productos_migrados registros<br>";
        ...
        $stmt->execute();
        $imagenes_migradas = $stmt->rowCount();
        echo "✅ Imágenes migradas: $imagenes_migradas registros<br>";
        ...
        $stmt->execute();
        $documentos_migrados = $stmt->rowCount();
        echo "✅ Documentos migrados: $documentos_migrados registros<br>";
...
echo "<h1>🚀 IMPORTANDO DATOS DEL SISTEMA VIEJO AL CATÁLOGO (V3)</h1>";
echo "<hr>";
